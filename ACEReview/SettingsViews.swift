...
struct AccountDeletionInformationView: View {
    var body: some View {
        SettingsScreen(title: "账号注销", subtitle: "注销是不可逆操作。提交前请先下载或保存仍需使用的报告。") {
            ...
            infoPanel(icon: "clock.fill", title: "处理方式", text: "为防止冒用，注销前需要验证当前账号并检查未完成的上传、分析和组织责任。正式申请入口需由运营主体启用后才能提交。")
            VStack(alignment: .leading, spacing: 10) {
                Text("发布前配置尚未完成").font(.headline).foregroundStyle(ACETheme.cardInk)
                Text("当前安装包尚未配置运营主体名称、隐私联系渠道和服务端注销接口，因此不会提供虚假的“已提交”按钮。正式商用构建必须补齐后启用。")
...
struct SupportView: View {
    ...
    var body: some View {
        SettingsScreen(title: "帮助与支持", subtitle: "先快速定位上传、分析和报告问题；联系支持时可附上任务名称与时间。") {
            ...
            Text("隐私联系与客户支持渠道需在正式发布配置中填写。").font(.caption).foregroundStyle(ACETheme.muted).frame(maxWidth: .infinity)
        }
    }
...
struct PrivacyPolicyView: View {
    ...
    private var privacySections: [LegalSection] {[
        .init("一、适用范围与运营者", "本政策适用于 ACE Review iOS 应用及配套云端分析服务。正式发布版本应在此处公示运营主体全称、注册地址、个人信息保护负责人及可验证的联系渠道；这些法定信息不得以产品名称代替。"),
        ...
        .init("五、共享、委托处理与转移", "除为提供云计算、对象存储、内容分发、消息通知、安全防护等必要能力而委托服务商处理，或法律法规另有要求外，我们不会出售个人信息。正式发布前应公示实际服务商名称、处理目的、数据种类、服务器地域和安全措施。发生合并、分立或资产转让时，将依法告知接收方并要求其继续受本政策约束。"),
        .init("六、保存地点与期限", "个人信息保存在提供服务所必需的地域和期限内。训练视频、任务与报告的具体默认保存期限、备份清除周期和日志留存期限，应由运营主体在正式发布配置中明确。达到目的、保存期届满、账号注销或你依法要求删除后，我们将删除或匿名化；法律法规要求保留的，隔离保存且不再用于其他目的。"),
        ...
        .init("十一、政策更新与联系", "重大变更将通过应用内显著提示重新告知，依法需要同意的将再次征得同意。生效日期：2026年8月10日。正式商用前必须补齐运营主体、联系渠道、处理者清单和明确保存期限，并经适用地区的专业法律审查。")
    ]}
}
...
struct TermsOfServiceView: View {
    ...
    private var sections: [LegalSection] {[
        ...
        .init("十、终止、法律与争议", "你可停止使用并按隐私政策申请注销。协议终止不影响终止前已产生的合法权利义务。适用法律、争议解决机构及运营主体住所地必须由正式运营主体在发布前依法明确，不得以空白或产品名称替代。生效日期：2026年8月10日。")
    ]}
}
